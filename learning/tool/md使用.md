...
- ~ 夹 ~包围打出下标,^ 夹 ^包围打出上标
- 可以用<></>内联html,$ $内联latex,但**放在代码块里**的不会被渲染

## Github风格的Alert / Callouts
...
创建一个让读者可自行选择是否要展开折叠部分内容进行详细阅读。例如，当您希望在问题评论中包含技术细节时，这些细节可能与每个读者都不相关或不感兴趣，您可以将这些细节放在折叠部分中。

在读者单击展开详细信息之前，`<details>`块中的任何Markdown都将被折叠，在`<details>`块中，使用`<summary>`标签让读者知道里面是什么。标签出现在>的右边。
...
![img](./md%E4%BD%BF%E7%94%A8/v2-8a5c4c82ffa2f4b1543adb14c584c873_1440w.jpg)
...
![img](./md%E4%BD%BF%E7%94%A8/v2-3ef78e260ee0f9b77a19a22f746a8511_1440w.jpg)

### 创建Mermaid 流程图

Mermaid是一个受markdown启发的工具，可以**将文本呈现为图表**。

要创建Mermaid图，请在带有Mermaid语言标识符的隔离代码块中**添加Mermaid语法**。
...
| 粘贴为纯文本                           | Ctrl + Shift + V         |
| 选择行/句子 选择行（甚至在**表格**中） | Ctrl + L                 |
| 添加行（在表中）                       | Ctrl + Enter             |
| 删除行（在表中）                       | Ctrl + Shift + Backspace |
| 选择**单词**                           | Ctrl + D                 |
| 删除单词                               | Ctrl + Shift + D         |
| 仅查找                                 | Ctrl + F                 |
| 代替                                   | Ctrl + H                 |
